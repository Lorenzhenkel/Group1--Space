...
#define vd_heating_pin 16        //Heater
#define vd_cooling_pin 17       //Cooler
...
void setup(void) {
 ...
 Setpoint = 19;//get_target_temp(temp_outside);
...
void loop(void) {
 ...
 Serial.println(myPID.Compute());
  Input = temp_inside;
   ...
   analogWrite(vd_heating_pin, Output);

  Serial.print("PID ");
  Serial.println(Output);
  delay(1000);

  if (Output >= 127 && Output != 255) {
    digitalWrite(vd_cooling_pin, LOW);
    digitalWrite(vd_heating_pin, HIGH);
  }
  else if(Output < 127) {
    digitalWrite(vd_heating_pin, LOW);
    digitalWrite(vd_cooling_pin, HIGH);
  }
  else {
       digitalWrite(vd_heating_pin, LOW);
    digitalWrite(vd_cooling_pin, LOW);
...
float get_target_temp(float temp_outside){
  double var;
  if (temp_outside > 3.33)
  {
    var = 19;
  }
  else
  {
    var = tempFunction(temp_outside);
  }
  return var;
}
*/
